...
import { useEvent } from "../src/index.tsx";
import { ListenerType1, ListenerType2 } from "../src/useEvent.tsx";
...
test("should call addEventListener/removeEventListener on mount/unmount", () => {
	checkOnMountAndUnmount(
		propsList1[0] as Props,
		"addEventListener",
		"removeEventListener",
	);
});

test("should call on/off on mount/unmount", () => {
	checkOnMountAndUnmount(propsList2[0] as Props, "on", "off");
});

test("should call addEventListener/removeEventListener on deps changes", () => {
	checkOnDepsChanges(
		propsList1[0] as Props,
		propsList1[1] as Props,
		"addEventListener",
		"removeEventListener",
	);
});

test("should call on/off on deps changes", () => {
	checkOnDepsChanges(
		propsList2[0] as Props,
		propsList2[1] as Props,
		"on",
		"off",
	);
});
