...
import { KSUID, base62, debase62 } from "../src/ksuid.ts";
...
test("array buffer values can be base62 encoded and decoded", () => {
...
test("lexographic ordering of encoded values is that of decoded values", () => {
	const strings = [];
	for (let i = 0; i < 256; i++) {
...
test("created with the current time", () => {
	const x = KSUID.random();
	expect(x.date.valueOf()).toBe(14e11);
});

test("provides the uncorrected timestamp in seconds", () => {
...
test("random accepts milliseconds value", () => {
	const ms = new Date(2025, 4, 15).getTime();
...
test("throws if called without valid buffer", () => {
...
test("buffer accessor returns new buffers", () => {
...
test("raw accessor returns raw buffer", () => {
...
test("payload accessor returns payload buffer", () => {
...
test("payload accessor returns new buffers", () => {
	const expected = Buffer.alloc(16, 0xff);
	const x = new KSUID(
		toArrayBuffer(Buffer.concat([Buffer.alloc(4), expected])),
	);
...
test("compare() returns 0 when comparing against a non-KSUID", () => {
	// @ts-expect-error Testing invalid input
...
test("KSUID.fromParts() validates timeInMs", () => {
	let notInt = "";
	try {
...
test("KSUID.fromParts() validates payload", () => {
	let notInt = "";
	try {
		// @ts-expect-error Testing invalid input
