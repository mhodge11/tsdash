...
import type { Prettify, Stringifiable } from "../internal/index.ts";
import type { Numbers } from "../numbers/index.ts";
import type * as NumberImpls from "../numbers/internal/index.ts";
import type { Objects } from "../objects/index.ts";
import type { Std } from "../std/index.ts";
import type { Iterators } from "./internal/index.ts";
...
export namespace Tuples {
	...
	export type MinLength<T, L extends number> = BuildMinLength<T, L>;

	type BuildMinLength<
		T,
		L extends number,
		I extends T[] = [],
	> = I["length"] extends L ? [...I, ...T[]] : BuildMinLength<T, L, [...I, T]>;
	...
	type DropImpl<
		...
		n extends any[],
	> = Iterators.Get<n> extends 0
		? xs
		: xs extends readonly [any, ...infer tail]
		  ? DropImpl<tail, Iterators.Prev<n>>
		  : [];
	...
	export interface DropFn extends Fn {
		return: this["args"] extends [
			...
			? DropImpl<T, Iterators.Iterator<N>>
			: never;
	}
	...
	type TakeImpl<
		...
		it extends any[],
		output extends any[] = [],
	> = Iterators.Get<it> extends 0
		? output
		: xs extends readonly [infer head, ...infer tail]
		  ? TakeImpl<tail, Iterators.Prev<it>, [...output, head]>
		  : output;
	...
	interface TakeFn extends Fn {
		return: this["args"] extends [
			...
			? TakeImpl<T, Iterators.Iterator<N>>
			: never;
	}
	 ...
	 * ```
	 */
	export interface Sort<predicateFn extends Fn = Numbers.LessThanOrEqual>
		extends Fn {
	...
	export interface SplitAtFn extends Fn {
		return: this["args"] extends [
			...
			? [
					TakeImpl<tuple, Iterators.Iterator<index>>,
					DropImpl<tuple, Iterators.Iterator<index>>,
			  ]
			: never;
	}
