import type { CompareFunction } from "@helpers/types.ts";
import type { ArrayMinLength } from "@type/ArrayMinLength.ts";

import { fastFlat } from "@helpers/fastFlat.ts";
...
export function intersection<U>(
	...arraysOrCompareFn: ArrayMinLength<U[], 2>
): U[];
export function intersection<T extends ArrayMinLength<unknown[], 2>>(
	...arraysOrCompareFn: [...T, CompareFunction<T>]
): T[0];
export function intersection<T extends ArrayMinLength<unknown[], 2>, U>(
	...arraysOrCompareFn: ArrayMinLength<U[], 2> | [...T, CompareFunction<T>]
): T[0] {
	...
	const combinedRestArray = fastFlat(arrays);
